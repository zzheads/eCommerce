...
public class CheckoutController {
	...
	String checkoutCoupon(Model model) {
		...
		if (!model.containsAttribute("couponCode")) {
			couponCode = sCart.getCouponCode();
		} else {
			couponCode = (CouponCode) model.asMap().get("couponCode");
		}
	...
	// @ModelAttribute(value="couponCode")
	@RequestMapping(path="/coupon", method = RequestMethod.POST)
	String postCouponCode(Model model, @Valid CouponCode couponCode, BindingResult result, RedirectAttributes redirectAttributes) {

		// coupon code cant be empty, must be 5-10 chars length
		if(result.hasErrors()) {
